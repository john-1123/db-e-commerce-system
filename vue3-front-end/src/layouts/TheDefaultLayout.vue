...
<template>
  <v-layout>
    <v-app-bar color="grey-darken-2">
      ...
      <v-btn to="/order" v-if="authStore.state.isLogin">Order</v-btn>
      <v-btn to="/manage/product" v-if="authStore.state.isLogin"
        >Manage Product</v-btn
      >
      <v-btn to="/manage/order" v-if="authStore.state.isLogin"
        >Manage Order</v-btn
      >
